...
class EditRecipeScreen extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => EditRecipeScreenState();
}
...
class EditRecipeScreenState extends State<EditRecipeScreen> {
  ...
  var switchIsPrivate = 0; //this is the recipe privacy switch

  @override
  void initState() {
    super.initState();
    _isPublic = widget.r.public;
    ingredients = widget.r.ingredients;
    steps = widget.r.steps;
    _titleController.text = widget.r.title;
    _descriptionController.text = widget.r.description;
    cookTime = widget.r.cookTime;
    prepTime = widget.r.prepTime;
    activeTime = widget.r.activeTime;
    totalTime = widget.r.totalTime;
    selectedType = widget.r.type;
    characteristics = widget.r.characteristics;
    recipeLinks = widget.r.recipeLinks;
    _summaryController.text = widget.r.summary;
    _videoLinkController.text = widget.r.videoLink;
    pairsWith = widget.r.pairsWith;
    _imageUrlController.text = widget.r.imageUrl;
    _isPublic = widget.r.public;
    switchIsPrivate = widget.r.public == true ? 1 : 0;
    _sourceController.text = widget.r.source;
    _nutritionController.text = widget.r.nutrition;
    _equipmentController.text = widget.r.equipment;
    _servingsController.text = widget.r.servings.toString();
    ...
    return Scaffold(
        appBar: AppBar(
          ...
          actions: [
            TextButton(
                onPressed: () async {
                  List<Recipe> recipes = await GetIt.I<RecipeDataService>().getAllRecipes();
                  ...
                  myRecipes.remove(widget.r.title.toLowerCase());

                  if (!myRecipes.contains(_titleController.text.toLowerCase())) {
                    await widget.onEditRecipe(
                      Recipe(
                    ...
                    if (image != null) {
                      await GetIt.I<RecipeDataService>().uploadImageToDatabase(image, widget.r.uid);
                    }
                  } else {
                    widget.showSnackBar();
                  }
                ...
                .map((e) => TextCard(
                      ...
                      onDelete: () {
                        setState(() {
                          ...
                          // print("mycurrent order: ");
                          // print(widget.steps.indexOf(e));
                        });
                      },
                      onEdit: () {
                        _stepController = TextEditingController();
